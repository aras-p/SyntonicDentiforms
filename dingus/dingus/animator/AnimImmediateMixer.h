...
public:
	...
	/// Blend the streams into provided destination array. Does NOT normalize weights.
	void update( int numCurves, value_type* dest, int destStride ) {
		...
		for( i = 0; i < n; ++i ) {
			...
			s.mStream->update( numCurves, &mScratchValues[0], sizeof(value_type) );
