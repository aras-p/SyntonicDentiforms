...
struct Vector3
{
  public:
    ...
    Vector3 transformCoord(const Matrix4x4 &pm) const;
...
class AABox
{
  public:
    ...
    bool frustumCull(const Matrix4x4 &world, const Plane planes[6]) const;
...
class Line3
{
  public:
    Line3() {};
    Line3(const Vector3 &v0, const Vector3 &v1) : pos(v0), vec(v1 - v0) {};
    ...
    const Vector3 &getStart() const { return pos; }
    const Vector3 &getVector() const { return vec; }
    Vector3 getEnd() const { return (pos + vec); };
    ...
    float project(const Vector3 &p) const
    {
        Vector3 diff(p - pos);
        float l = vec.lengthSq();
        assert(l > 1.0e-6f);
    ...
    Vector3 interpolate(float t) const { return pos + vec * t; }
...
struct Matrix4x4
{
  public:
    ...
    Matrix4x4(const Vector3 &pos, const Quaternion &rot);
...
inline Matrix4x4::Matrix4x4(const float *f) { memcpy(&_11, f, sizeof(*this)); }
inline Matrix4x4::Matrix4x4(float f11, float f12, float f13, float f14,
...
inline const Vector3 &Matrix4x4::getAxisX() const { return *(Vector3 *)&_11; };
inline Vector3 &Matrix4x4::getAxisX() { return *(Vector3 *)&_11; };
inline const Vector3 &Matrix4x4::getAxisY() const { return *(Vector3 *)&_21; };
inline Vector3 &Matrix4x4::getAxisY() { return *(Vector3 *)&_21; };
inline const Vector3 &Matrix4x4::getAxisZ() const { return *(Vector3 *)&_31; };
inline Vector3 &Matrix4x4::getAxisZ() { return *(Vector3 *)&_31; };
inline const Vector3 &Matrix4x4::getOrigin() const { return *(Vector3 *)&_41; };
inline Vector3 &Matrix4x4::getOrigin() { return *(Vector3 *)&_41; };

inline void Matrix4x4::identify()
{
    m[0][1] = m[0][2] = m[0][3] =
        m[1][0] = m[1][2] = m[1][3] =
            m[2][0] = m[2][1] = m[2][3] =
...
struct Vector4
{
  public:
    ...
    {
        uint32_t dwR = x >= 1.0f ? 0xff : x <= 0.0f ? 0x00
                                                    : (uint32_t)(x * 255.0f + 0.5f);
        uint32_t dwG = y >= 1.0f ? 0xff : y <= 0.0f ? 0x00
                                                    : (uint32_t)(y * 255.0f + 0.5f);
        uint32_t dwB = z >= 1.0f ? 0xff : z <= 0.0f ? 0x00
                                                    : (uint32_t)(z * 255.0f + 0.5f);
        uint32_t dwA = w >= 1.0f ? 0xff : w <= 0.0f ? 0x00
                                                    : (uint32_t)(w * 255.0f + 0.5f);
