...
void gRenderWallReflections()
{
    ...
    for (int currWall = 0; currWall < CFACE_COUNT; ++currWall)
    {
        ...
        sg_view resolve_views[CFACE_COUNT] = {
            rt_refl_px.view_resolve,
            rt_refl_nx.view_resolve,
            rt_refl_py.view_resolve,
            rt_refl_ny.view_resolve,
            rt_refl_pz.view_resolve,
            rt_refl_nz.view_resolve,
        };
...
static void gRenderCredits(float cutAlpha)
{
    ...
    for (i = 0; i < BILLCOUNT; ++i)
    {
        ...
        {
            b->tu1 = 0.0f;
            b->tv1 = line * 2 * UV_HEIGHT;
            b->tu2 = 850.0f / 1024.0f;
            b->tv2 = (line * 2 + 1) * UV_HEIGHT;
        }
        else
        {
            b->tu1 = 0.0f;
            b->tv1 = line * UV_HEIGHT;
            b->tu2 = 0.5f;
            b->tv2 = (line + 1) * UV_HEIGHT;
        }
        ...
        // right...
        b = &billboards_add();
        b->x1 = CENTER_X + edgeBill;
        b->y1 = TOP_Y + SECOND_BILL_LOWER;
        b->x2 = b->x1 + WIDTH;
        b->y2 = b->y1 + HEIGHT;
        if (outer)
        {
            b->tu1 = 0.0f;
            b->tv1 = (line * 2 + 1) * UV_HEIGHT;
            b->tu2 = 850.0f / 1024.0f;
            b->tv2 = (line * 2 + 2) * UV_HEIGHT;
        }
        else
        {
            b->tu1 = 0.5f;
...
void demo_event(const sapp_event *evt)
{
    ...
    if (evt->type == SAPP_EVENTTYPE_KEY_DOWN)
    {
        ...
        if (evt->key_code == SAPP_KEYCODE_RIGHT)
            gDebugTime += dt * 1.0f;
        if (evt->key_code == SAPP_KEYCODE_PAGE_UP)
            gDebugTime -= dt * 5.0f;
        if (evt->key_code == SAPP_KEYCODE_PAGE_DOWN)
            gDebugTime += dt * 5.0f;
